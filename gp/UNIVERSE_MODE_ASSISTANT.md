...
- Delete with confirm
- **Bulk select mode** — toggled by the "Select" button in the header. Adds checkboxes to every wrestler (cards & list views), shows a sticky toolbar with "Select all visible" + a per-show row (Raw / SmackDown / Free Agent) plus Delete. Bulk-assigning updates both `brand` and `showId` together. Toggling "Done" exits select mode and clears the selection.
- The edit dialog now derives its initial Show value from the wrestler's `brand` when no explicit `showId` is set (RAW → Raw, SMACKDOWN → SmackDown). Saving with "Free Agent" selected only flips to FREE_AGENT for wrestlers that previously had a show-mapped brand — NXT and existing Free Agents keep their brand, so a passive edit no longer demotes them.

Wrestler photos are now bundled with the app at `artifacts/universe-mode/public/wrestlers/`. The seedRoster references each photo via an `imageUrl` field that resolves to the public-folder URL. 197 wrestlers in the seed have bundled photos. Users can still upload custom replacement images via the Roster UI — custom uploads override seed photos. Photos persist across devices and browser cache clears because they're part of the app bundle, not localStorage.
...
- `umc.rosterPhotosMigratedVersion` — version number (currently `"2"`) for the bundled-photo back-fill. When new wrestlers gain `imageUrl` in the seed, bump `CURRENT_PHOTO_VERSION` in `useRoster` so existing users re-run the migration once. The legacy `umc.rosterPhotosMigrated` flag is treated as version `1`.
...
#### Color exception — NEWS tab
The NEWS tab is the single sanctioned break from the monochrome rule. Two complementary aesthetics coexist:

- **Newsstand page** (`News.tsx`) — WWE.com 2003-2004 "scratch logo" era: dark steel-and-red palette, hero panel with featured wrestler photo full-bleed, headlines sidebar for recent rumors, story panels row for issue features, and a "On The Rack" back-issues strip. The newsstand page itself uses a WWE.com 2003-2004 'scratch logo' aesthetic — dark steel-and-red palette, hero panel with featured photo, headlines sidebar, story panels, and back issues strip. The IssueView and RumorView popups retain the WWF Magazine cover aesthetic — two complementary styles for two contexts.
- **IssueView + RumorView popups** — retain the original Russo-era WWF Magazine cover aesthetic (cream paper textures, yellow/blue/red palette, halftone/grain overlays).

Sanctioned colors inside `.news-tab`:
  - `#0a0a0a` near-black primary background
  - `#1a1a1a` to `#2a2a2a` panel gradients
  - `#3a3a3a` chrome/silver borders and brushed metal
  - `#dc1e1e` brand red, primary accent
  - `#8a0000` deep blood red, scratch strokes and aggressive accents
  - `#f4c41a` yellow — used sparingly, BREAKING tags and burst stickers only
  - `#ffffff` white text on photos and primary headlines
  - `#9a9a9a` muted gray for secondary text
  - Plus magazine-interior colors (`#1e4ebf`, `#f4ecd8`, `#3ecf6b`) inside IssueView/MagazineCover only

Outside `.news-tab`, monochrome + brand-red still rules everything.
...
- **2026-04-30 — BUNDLED WRESTLER PHOTOS.** Bundled 197 wrestler photos into the app at `artifacts/universe-mode/public/wrestlers/` so portraits survive across devices and browser cache clears. Renamed 13 typo'd filenames to match the seed slugs (e.g. `aliester=black.png` → `aleister-black.png`, `damien-priest.png` → `damian-priest.png`, `cody-rhodes.png` → `american-nightmare-cody-rhodes.png`, `the-undertaker.png` → `undertaker.png`). Added `imageUrl: "/wrestlers/<slug>.<ext>"` to the matching entries in `seedRoster.ts` — **195 of 257 seed wrestlers** now have bundled photos (the unmatched 62 are mostly the women's division, whose photos weren't in the upload, plus Sid Justice and D'Lo Brown which had filename mismatches). Replaced the previously-escaped `Seth "Freakin" Rollins` seed entry with a clean `"Seth Rollins"` entry so it matches `seth-rollins.png`. New one-time localStorage migration in `useRoster` (gated by the new `umc.rosterPhotosMigrated` flag) iterates the existing user's roster and back-fills `imageUrl` from the seed by case-insensitive name match — only when the user's wrestler has NO `imageUrl` set, so custom uploads (data URIs) are always preserved. Roster UI already consumed `w.imageUrl` with an initials fallback, so no UI changes were needed. OpenAPI spec untouched (the `Wrestler.imageUrl` field already existed). Two files left in place without seed matches and logged for the user: `sid-vicious.png` and `d-lo-brown.png`.

- **2026-04-30 — NEWS TAB PHASE 2 (auto-issue, cover-story filing, magazine context loop).** Three coordinated additions on top of the Phase 1 newsstand. (1) **Auto-issue on week advance.** `UniverseClock.handleAdvance` now compares `weekKey(prior)` vs `weekKey(next)` (new helper in `calendar.ts`, format `"y-m-w"`); when they differ AND `umc.history` has at least one `kind:"log"` whose `universeDate.weekKey === priorKey` AND no existing issue with that key, it fires `useGenerateIssue` with `buildBookerContext(..., issues)` and `prior` as the universe date. The issue is dated to the recapped week and added to `umc.issues` with `read:false`. Failures are silent (the user can still print manually). The `AppHeader` NEWS tab now shows a 7px `#dc1e1e` red dot with a 1px background-color ring whenever any issue is unread — the only sanctioned color exception outside the news-tab itself. The News tab additionally renders a `bg-muted/40` notification strip ("NEW ISSUE(S) ON THE STAND") with a DISMISS button that marks every unread issue as read. (2) **File Cover Story.** New `kind:"magazine"` `RivalryEntry` (data: `{issueId, issueNumber, coverHeadline, coverFeaturedWrestlerId?}`) plus a "FILE COVER STORY" footer button in `IssueView` that opens `FileToRivalryDialog` in the new `mode="issue-cover"`. In that mode the dialog title becomes "FILE COVER STORY?", a small cover preview is rendered above the rivalry list, and `ensureEntryInHistory()` upserts the magazine entry into `umc.history` before linking it (idempotent on `entry.id = "magazine-<issueId>"`). The dialog still consumes `IssueCoverData.suggestedRivalryHint` so the AI's best guess pre-selects a rivalry. Magazine entries render in `RivalryDetail.EntryCard`, `Rivalries.FeedCard`, and `Rivalries.FeedView` (with a new `MAGAZINES` legacy filter pill); `entryParticipantNames` returns `[]` for magazine and `entryKindForContext` / `entrySummary` cover the new kind. (3) **Magazine context loop.** New `MagazineHeadline {issueNumber, coverHeadline, weekKey?}` schema; `BookerRequest`, `PromoRequest`, `BookerChatRequest`, `RivalryAnalyzeRequest`, and `IssueRequest` all gained an optional `recentMagazineHeadlines: MagazineHeadline[]`. Server `buildContext` formats them into a `RECENT MAGAZINE HEADLINES` block. `IssueRequest`/`IssueCoverData` gained `suggestedRivalryHint` so the booker can hint a real rivalry to file the cover under (HINT_RULES-style guidance in the system prompt). `buildBookerContext` now takes a 9th `issues: Issue[] = []` arg and ships the last 3 issues' headlines + week keys; every callsite updated (News.tsx ×2, Chat.tsx, CreativeDesk.tsx ×2, RivalryDetail.useAnalyzeBaseContext, UniverseClock auto-issue path). OpenAPI codegen rerun, no new dependencies, monochrome rule preserved (red dot is the only exception).

- **2026-04-30 — NEWS TAB OVERHAUL (WWE.com 2003 scratch-logo aesthetic).** Complete visual redesign of `News.tsx` and the news-tab CSS in `index.css`. The newsstand page now uses a WWE.com 2003-2004 "scratch logo" era look: brushed-metal header strip with the Gorilla Position scratch logo SVG + chrome-gradient action buttons; full-bleed hero panel with wrestler photo background (contrast/saturation boosted, dark gradient overlay), giant italic Oswald headline with blood-red text-shadow, and a "Read Issue" chrome button; right-side Headlines sidebar with thumbnail + headline rumor cards separated by thin red rules; a story panels row below showing features from the latest issue as photo-backed cards with scratch-style headlines; and an "On The Rack" horizontally-scrollable back issues strip using the existing `MagazineCover` at `size="thumb"`. New static SVG assets added to `artifacts/universe-mode/public/news/`: `scratch-logo.svg`, `scratch-accent-divider.svg`, `brushed-metal.svg`. Background is now dark steel (`#0a0a0a` → `#111111`), not brown. `MagazineCover`, `IssueView`, and `RumorView` are unchanged — the WWF Magazine aesthetic is preserved for inside-the-issue viewing. Typecheck passes.
